...
public class ClosedIntervalTest {
    ...
    void ofWorksWithValidValues() {
        ...
        for (int i = 0; i < TestRandomizer.RANDOM_ITERATIONS; i++) {
            ...
            assertEquals(low, interval.low(), 1e-9);
            assertEquals(low + size, interval.high(), 1e-9);
            assertEquals(size, interval.size(), 1e-9);
        }
    }
    ...
    void symmetricWorksWithValidValues() {
        ...
        for (int i = 0; i < TestRandomizer.RANDOM_ITERATIONS; i++) {
            ...
            assertEquals(-size / 2d, interval.low(), 1e-9);
            assertEquals(size / 2d, interval.high(), 1e-9);
            assertEquals(size, interval.size(), 1e-9);
        }
    }
